...
def test_valid_galactrum_address():
    from galactrumlib import is_valid_galactrum_address

    main = valid_galactrum_address()
    test = valid_galactrum_address('testnet')

    assert is_valid_galactrum_address(main) is True
    assert is_valid_galactrum_address(main, 'mainnet') is True
    assert is_valid_galactrum_address(main, 'testnet') is False

    assert is_valid_galactrum_address(test) is False
    assert is_valid_galactrum_address(test, 'mainnet') is False
    assert is_valid_galactrum_address(test, 'testnet') is True


def test_invalid_galactrum_address():
    from galactrumlib import is_valid_galactrum_address

    main = invalid_galactrum_address()
    test = invalid_galactrum_address('testnet')

    assert is_valid_galactrum_address(main) is False
    assert is_valid_galactrum_address(main, 'mainnet') is False
    assert is_valid_galactrum_address(main, 'testnet') is False

    assert is_valid_galactrum_address(test) is False
    assert is_valid_galactrum_address(test, 'mainnet') is False
    assert is_valid_galactrum_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from galactrumlib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % galactrumlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(galactrumlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(galactrumlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(galactrumlib.blocks_to_seconds(16616)) == 2612035
